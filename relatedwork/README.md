...
| `Table2-data.csv` | One row per study: binary flags for which comprehension measure types (physiological, correctness, time, subjective rating, aggregation, others) and task categories (Tc1-Tc4) it used. Aggregated into Table 2 (measure type frequency) — no script; sum the measure-type columns yourself, or see the Additional Materials section of the top-level README. |
